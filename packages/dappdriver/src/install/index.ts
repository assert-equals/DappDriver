...
import { Artifact, Asset } from '../types';
...
export function compareVersion(wallet: string, version: string, recommendedVersions: Range): void {
...
export async function fetchGithubRelease(
  ...
  if (!release) {
    throw new Error(`Could not find version (${version}) of ${wallet}, try updating the ${packageName} package.`);
  }
  return release;
}
